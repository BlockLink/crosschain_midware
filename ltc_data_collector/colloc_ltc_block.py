...
def do_collect_app(db):
    while True:
        try:
            #程序启动，设置为同步状态
            config = db.b_config
            config.update({"key":"ltcsyncstate"},{"key":"ltcsyncstate","value":"true"})


            # 清理上一轮的垃圾数据，包括块数据、交易数据以及合约数据
            GlobalVariable_ltc.last_sync_block_num = clear_last_garbage_data(db)
            GlobalVariable_ltc.sync_limit_per_step = 10
            ...
            while True:
                ...
                if GlobalVariable_ltc.last_sync_block_num >= latest_block_num:
                    GlobalVariable_ltc.sync_start_per_round = latest_block_num
                    GlobalVariable_ltc.sync_end_per_round = latest_block_num
                else:
                    GlobalVariable_ltc.sync_start_per_round = GlobalVariable_ltc.last_sync_block_num
                    GlobalVariable_ltc.sync_end_per_round = ((
                                                         GlobalVariable_ltc.last_sync_block_num + SYNC_BLOCK_PER_ROUND) >= latest_block_num) \
                                                        and latest_block_num or (
                                                        GlobalVariable_ltc.last_sync_block_num + SYNC_BLOCK_PER_ROUND)
                logging.debug("This round start: %d, this round end: %d" % (GlobalVariable_ltc.sync_start_per_round, GlobalVariable_ltc.sync_end_per_round))

                sync_rate = float(GlobalVariable_ltc.sync_start_per_round) / latest_block_num
                sync_process = '#' * int(40 * sync_rate) + ' ' * (40 - int(40 * sync_rate))
                sys.stdout.write(
                    "\rsync block [%s][%d/%d], %.3f%%\n" % (sync_process, GlobalVariable_ltc.sync_start_per_round,
                                                          latest_block_num, sync_rate * 100))
                while GlobalVariable_ltc.sync_start_per_round <=GlobalVariable_ltc.sync_end_per_round:
                    logging.debug("Start collect step from %d" % GlobalVariable_ltc.sync_start_per_round)
                    collect_data_cb(db)
                    GlobalVariable_ltc.last_sync_block_num = GlobalVariable_ltc.sync_start_per_round
                    config.update({"key": "ltcsyncblocknum"}, {"$set":{"key": "ltcsyncblocknum", "value": str(GlobalVariable_ltc.last_sync_block_num)}})

                if GlobalVariable_ltc.sync_start_per_round == latest_block_num + 1:
                    break
...
def get_latest_block_num(db):
    ret = ltc_request("getblockcount",[])
    real_block_num = ret['result']
    safe_block = 6
    safe_block_ret = db.b_config.find_one({"key":"ltcsafeblock"})
...
def clear_last_garbage_data(db_pool):
    ...
    ret = config.find_one({"key":"ltcsyncblocknum"})
    if ret is None:
        return 0
    last_sync_block_num = int(ret["value"])
    try:
        db_pool.b_raw_transaction.remove({"blockNum":{"$gte":last_sync_block_num},"chainId":"ltc"})
        db_pool.b_block.remove({"blockNumber":{"$gte":last_sync_block_num},"chainId":"ltc"})
        db_pool.b_raw_transaction_input.remove({"blockNum": {"$gte": last_sync_block_num},"chainId":"ltc"})
        db_pool.b_raw_transaction_output.remove({"blockNum": {"$gte": last_sync_block_num},"chainId":"ltc"})
        db_pool.b_deposit_transaction.remove({"blockNum": {"$gte": last_sync_block_num},"chainId":"ltc"})
        db_pool.b_withdraw_transaction.remove({"blockNum": {"$gte": last_sync_block_num},"chainId":"ltc"})
    except Exception,ex:
        print ex
...
def collect_block( db_pool, block_num_fetch):
    ret1 = ltc_request("getblockhash",[block_num_fetch])
    if ret1['result'] == None:
        raise Exception("blockchain_get_block error")
    block_hash = ret1['result']
    ret2 = ltc_request("getblock",[str(block_hash)])
    if ret2['result'] == None:
        raise Exception("blockchain_get_block error")
    json_data = ret2['result']
    block_info = BlockInfoLtc()
...
def get_transaction_data(trx_id):
    ...
    ret = ltc_request("getrawtransaction",[trx_id, True])
        ...
        if in_trx is None:
            ...
            for t in in_trx["vout"]:
                if t["n"] == trx_in["vout"] and t["scriptPubKey"].has_key("addresses"):
                    ...
                    if db_pool.b_ltc_multisig_address.find_one({"address": in_address, "addr_type": 0}) is not None:
                        if multisig_in_addr == "":
    ...
    for trx_out in vout:
        if trx_out["scriptPubKey"].has_key("addresses"):
            ...
            if (out_set.has_key(out_address)):
                ...
                out_set[out_address] = trx_out["value"]
            if db_pool.b_ltc_multisig_address.find_one({"address": out_address, "addr_type": 0}) is not None:
...
def collect_data_cb(db_pool):
    try:
        count = 0
        while GlobalVariable_ltc.sync_start_per_round <= GlobalVariable_ltc.sync_end_per_round and count < GlobalVariable_ltc.sync_limit_per_step:
            block_num_fetch = GlobalVariable_ltc.sync_start_per_round
            ...
            GlobalVariable_ltc.sync_start_per_round += 1
            count += 1
