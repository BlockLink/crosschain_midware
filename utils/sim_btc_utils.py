...
class sim_btc_utils:
    ...
    def sim_is_coinbase(self,vins):
        if len(vins) == 1 and vins[0].has_key("coinbase"):
            return True
        return False

    def sim_btc_query_transacation(self,trxid):
        data = self.sim_btc_get_transaction(trxid)
        vin_result = []
        vout_result = []
        if data !="":
            if not self.sim_is_coinbase(data["vin"]):
                for vin in data["vin"]:
                    temp_trx_data = self.sim_btc_get_transaction(vin["txid"])
                    if temp_trx_data is not None:
                        for temp_vout in temp_trx_data["vout"]:
                            if temp_vout["n"] == vin["vout"]:
                                if temp_vout.has_key("value") and temp_vout["value"]>0:
                                    if temp_vout.has_key("scriptPubKey") and temp_vout["scriptPubKey"].has_key("addresses")     :
                                        vin_result.append({"address":temp_vout["scriptPubKey"]["addresses"],"value":temp_vout["value"]})
                                    else:
                                        vin_result.append({"address": "nostandard_address",
                                                           "value": temp_vout["value"]})
                                    break
            else:
                vin_result.append({"address": ["work_reward"],
                                                           "value": 0})
            for vout in data["vout"]:
                if vout.has_key("scriptPubKey") and vout["scriptPubKey"].has_key("addresses"):
                    vout_result.append(
                        {"address": vout["scriptPubKey"]["addresses"], "value": vout["value"]})
                else:
                    vout_result.append({"address": "nostandard_address",
                                       "value": vout["value"]})

            data["vin"] = vin_result
            data["vout"] = vout_result
            data.pop("hex")
            return data
        return ""
